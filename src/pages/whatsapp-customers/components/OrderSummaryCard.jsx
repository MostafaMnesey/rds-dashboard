import { Gift, Receipt, Truck } from "lucide-react";
import { memo } from "react";
import { useWatch } from "react-hook-form";
import { formatCurrency } from "../../orders/utils/formatters";
import { calculateShippingCost, calculateSubtotal } from "../utils";

const SummaryRow = ({ label, value, bold }) => (
  <div className="flex items-center justify-between">
    <span
      className={`text-sm ${
        bold ? "font-semibold text-soft-black" : "text-secondary"
      }`}
    >
      {label}
    </span>
...
const OrderSummaryCard = ({ control, shippingPrices, isLoadingShipping }) => {
  ...
  const discountRaw = useWatch({ control, name: "discount" });
  ...
  const discount = Math.max(Number(discountRaw) || 0, 0);
  const totalBeforeDiscount = subtotal + shipping.cost;
  const estimatedTotal = Math.max(totalBeforeDiscount - discount, 0);
